...
class EnSyncEngine {
  ...
  async createClient(accessKey) {
    try {
      ...
      if (data.startsWith("-FAIL:")) throw EnSyncError(data, "EnSyncGenericError");
  ...
  async publishEvent(eventName, payload = {}) {
    if (!this.config.clientId)
      throw Error("Cannot publish an event when you haven't created a client");
    try {
  ...
  async subscribeEvent(eventName, options = {}) {
    try {
      ...
      if (!eventName.trim() && typeof eventName !== "string")
        throw Error("EventName to subscribe to not passed");
