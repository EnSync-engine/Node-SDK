  ...
  describe("Message Key Distribution", function () {
    it("should encrypt and decrypt a message key", function () {
      ...
      assert.strictEqual(
        decryptedKey.length,
        messageKey.length,
        "Decrypted key should have same length"
      );
  ...
  describe("Hybrid Encryption", function () {
    ...
    it("should handle large messages with hybrid encryption", function () {
      ...
      for (const keyPair of recipients) {
        ...
        assert.strictEqual(
          decrypted,
          largeTestMessage,
          "Each recipient should be able to decrypt large message"
        );
      }
    });
    ...
    it("should fail for non-recipients", function () {
      ...
      assert.throws(
        ...
        "Should throw an error when non-recipient tries to decrypt"
      );
    });
    ...
    it("should be more efficient than traditional encryption for multiple recipients", function () {
      ...
      console.log(
        `Improvement: ${(((traditionalTime - hybridTime) / traditionalTime) * 100).toFixed(2)}%`
      );

      // Hybrid should be faster (allowing some margin for test environment variability)
      assert.ok(
        hybridTime < traditionalTime * 0.9,
      ...
      for (let i = 0; i < recipients.length; i++) {
        ...
        const traditionalDecrypted = crypto.decryptEd25519(
          traditionalEncrypted[i],
          keyPair.secretKey
        );
        assert.strictEqual(
          traditionalDecrypted,
          largeTestMessage,
          "Traditional decryption should work"
        );
    ...
    it("should support a complete encryption workflow", function () {
      ...
      for (const recipient of recipients) {
        ...
        assert.strictEqual(
          decrypted,
          originalMessage,
          "Recipient should be able to decrypt message"
        );
