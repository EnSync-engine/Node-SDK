...
args.forEach((arg, index) => {
  if (index === 0)
    accessKey = arg; // MEGLQzJucfsVB09Z8KZTfNSsMzpQ4zGY
  else if (index === 1)
    powerUsageEventName = arg; // utilityCompany/powerUsage
...
const powerSystem = async () => {
  ...
  while (true) {
    ...
    if (currentOutput > doNotExceed) {
      ...
      const tooHigh = await client.publish(powerUsageHighEventName, {
        "current.kWh": Math.round(currentOutput),
        dateTime: Date.now(),
      });
      await client.publish(powerUsageEventName, {
        "current.kWh": Math.round(currentOutput),
        dateTime: Date.now(),
      });
    ...
    // Normal power usage
    const normal = await client.publish(powerUsageEventName, {
      "current.kWh": Math.round(currentOutput),
      dateTime: Date.now(),
    });
